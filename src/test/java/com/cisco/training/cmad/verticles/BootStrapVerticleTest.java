...
import com.cisco.training.cmad.verticles.BootStrapVerticle;
...
public class BootStrapVerticleTest {
  ...
  public void setUp(TestContext context) throws IOException {
    ...
    vertx.deployVerticle(BootStrapVerticle.class.getName(), options, context.asyncAssertSuccess());
  }
  ...
  public void testMyApplication(TestContext context) {
    ...
    vertx.createHttpClient().getNow(port, "localhost", "/", response -> {
      response.handler(body -> {
        context.assertTrue(body.toString().contains("CMAD"));
        async.complete();
      });
    });
  }
